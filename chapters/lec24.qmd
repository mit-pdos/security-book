...
## Zero-knowledge proofs of knowledge {#sec-zkdefn}
...
$y=f(x)$.
We call such a prover-verifier interaction a *proof system*.
If the prover reveals no information about $x$, apart from the fact that $y=f(x)$,
to the verifier, we say that the proof system is a *zero-knowledge proof of knowledge*.
...
This is an *interactive proof*: the prover and
...
::: {.column-margin}
The
literature usually refers to the secret value $x$ as the *witness*.
:::

- *Completeness.* If the prover "knows" $x$,
  it will always convince an honest verifier to accept.

- *Soundness.* If the verifier accepts the
...
[**TODO (HCG):** Insert formal definitions here?]{.todo .content-visible when-profile="draft"}
...
A naïve definition might say that $x$
...
for *any* choice of the public function $f$, provided that it is
...
## Discrete-log problem and Schnorr signatures
...
## Reminder: The discrete-log problem
...
## Schnorr's protocol for proving knowledge of discrete logs
...
- The prover picks a random $r\in \Z_q$, and sends $R=g^r\in \G$ to the verifier.

- The verifier picks a challenge bit $c$ at random, either 0 or 1,
...
- The verifier checks that $g^z=R y^{c}\in \G$.  If equal, the verifier accepts,
...
we defined them in @sec-zkdefn.

The completeness follows by construction.
The more challenging steps are proving soundness and zero knowledge.

### Soundness using an extractor

We first need to convince ourselves that the protocol is *sound*: that is,
...
- The extractor runs the prover $P^*$ to get some initial protocol transcript with challenge $c=0$.
  The transcript consists of $(R, c=0, z)$.

- The extractor rewinds the state of the prover $P^*$ to just after the point when it
  sent the first protocol message $R$ to the verifier.

- The extractor runs $P^*$ again, but this time feeds its the challenge $c=1$.
...
### Zero-knowledge using a simulator
...
To do this, given a malicious verifier algorithm $V^*$, we construct a *simulator* algorithm.
The simulator's job
...
Crucially, in the simulated interaction, the simulator *does not know* the witness $x$,
while in the real prover-verifier interaction, the prover *does know* the witness $x$.
...
- Run the verifier $V^*$ on first message $R$.
  The verifier outputs a challenge $c$.

- If $c=c'$, output $(R, c, z)$ as the simulated transcript.
...
This is why the existence of an extractor does *not* mean that
in a real protocol interaction the verifier can extract the witness
$x$ from the prover.

## Fiat-Shamir heuristic and Schnorr signatures {#sec-hvzk}
...
The resulting signature scheme is called the *Schnorr signature scheme*.
