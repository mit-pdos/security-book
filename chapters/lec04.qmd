...
definition of "non-gibberish" we define.
...
fixed-size chunks $m_1, m_2, \ldots, m_{l}$.  Then,
...
::: {.column-margin}
The final monic term $k^l$ is essential, even though it
...
*every* key: the added chunks contribute nothing to the sum.
...
- Compute $M(v) \gets m_1 v + m_2 v^2 + m_3 v^3 + \cdots + m_{\ell} v^{\ell} + v^{\ell+1} \in \Z_p$.
...
More generally $m$ and $m \| 0^j$ would collide for every $j$.
...
- Now, say that the adversary finds a forged message-tag pair $(m^*, t^*)$.
  ...
  - Alternatively, the forger could use a random nonce $r^*$
    ...
    ::: {.column-margin}
    The added constant $z^* - z$ cannot cancel
