...
3. **Limiting damage.** In the cryptography part of this course, systems
...
::: {.column-margin}
When choosing what mechanism to use to isolate various components,
we think a lot about the *performance overhead* of an isolation mechanism.
...
These components will typically run on top of some *host* that enforces isolation. Importantly, this host must be correct! If there are bugs in the host, malicious code in a component may be able to exploit a bug to escape its isolation.
...
: Some common types of isolation {#tbl-isolationtypes}
...
::: {.column-margin}
Since all three of these actions start with the letters
"Au," we sometimes call this the *gold standard* for controlled sharing.
:::

- Authenticate: Associate the request with some *principal*.
  A principal could be a user name, an "origin" in the web context (e.g., `google.com`),
  a program, or some other entity in the system.
- Authorize: Decide whether that principal is allowed to make the request.
- Audit: Keep track of requests that each principal makes.
...
example in AFS, is via an *access control
...
- **Discretionary access control: "Owner" of each object sets the policy.**
...
- **Mandatory access control: Administrator sets policy.**
...
::: {.column-margin}
*Role-based access control* tries to hit some midpoint between discretionary
...
- It is difficult to keep policies up to date as the set of users evolves.
  Expiring permissions is one strategy.
- Users will complain if they do not have enough permissions, but they will
...
## Delegation and Chained Requests

Users often interact with systems *indirectly*.
...
A similar strategy can be seen, for example, in cloud file sharing: when you share a file in google drive, it generates a long random link that allows anyone with access to that link to view that file (and no others). This link itself becomes a *capability*---it allows anyone that posesses it to perform some related action.
