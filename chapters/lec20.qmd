...
bugs can have by *planning for compromise*
and aiming to limit the damage that each component
can cause if an attacker compromises it.

The philosophy that we will employ is called the *Principle of Least Privilege*.
...
::: {.column-margin}
In real systems, it is often costly to implement least privilege in
...
::: {.column-margin}
An orthogonal, but important, strategy is to reduce the number of privileges
that a component needs to do its job.
:::

**Challenge: Splitting Boundaries.**
...
::: {.column-margin}
If we don't split up a large piece of software
into components, the software just consists of one über-component.
An attacker who breaks into this one component can hijack the entire system.
:::

- **Isolating by user:** In an operating system, different users of the systems
  have different privileges. This way, if an attacker compromises
  one user's account it does not compromise the entire system.

- **Splitting by feature:** In large business applications, different features
...
- **Splitting off buggy code:** The Firefox web browser uses special sandboxes to compartmentalizes bug-prone video codec code (@sec-split-codec).
...
- **Separating exposed versus internal code:** Google's front-end HTTP servers
...
- **Complexity:** How much code does the isolation mechanism require?
...
*not* be allowed to remove entries from the
...
only API is `sign(msg)` and `get_public_key()`.
...
In particular, an attacker who compromises our app can still call `sign()` as much as they like to sign any message they like.
...
A common architecture for isolating cryptographic
...
## Example: Media Codecs in Web Browsers {#sec-split-codec}
...
The Firefox browser isolates media codecs
in a separate sandbox.
:::

Isolating codecs may not always be as trivial as @fig-codec makes it seem.
...
## Example: Server for Network Time Protocol (NTP)
...
the network cannot arbitrarily corrupt the system time.
:::
...
- a *signing* operation, that instructs the monitor to sign a protocol
  transcript,
- a *password-authentication* operation, that instructs
  the monitor to check a password (this can be rate limited to prevent
  password guessing), and
- a *start-session* operation, that instructs the monitor
...
The OpenSSH server is split into multiple process to mitigate the compromise of the network-facing code.
:::
...
Large Web services tend to isolate different application
...
and other suspect files from a separate domain ("origin')): `googleusercontent.com`.
Code loaded from `googleusercontent.com` cannot access cookies
or data for `google.com`. In this way, even if an attacker can somehow
...
Web browsers may isolate the execution of each origin's
...
Online vendors often use a separate payment processor
