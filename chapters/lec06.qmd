...
- RSA gives the only known instantiation of a *trapdoor one-way permutation*,
...
- RSA has a number of esoteric properties that are useful for advanced
  ...
  See
  [Boneh-Shoup, Chapter 10.9](https://toc.cryptobook.us/book.pdf#page=436) for details.

- RSA signatures are used on the vast majority of public-key certificates today.^[As
...
::: {#fig-rsa-owf-trapdoor layout="[45,55]"}
![](/figs/rsa-owf.svg)

![](/figs/rsa-trapdoor.svg)

A one-way function (at left) is hard to invert on random inputs.
...
::: {.column-margin}
If we wanted to be completely formal, the
input space would be parameterized by the security parameter $\lambda$.
So we would have a family of input spaces $\{\calX_\lambda\}_{\lambda \in \N}$---one
for each choice of $\lambda$. This way the input space can grow with $\lambda$.
:::
...
- $F(\pk, x) \to y$.
  ...
  ::: {.column-margin}
  In the RSA construction, the input space $\calX$
...
permutations of $\calX$.  Formally, for all $\lambda \in \N$,
$(\sk, \pk) \gets \Gen(1^\lambda)$, and $x, y \in \calX$:
$$ I(\sk, F(\pk, x)) = x \qquad\text{and}\qquad F(\pk, I(\sk, y)) = y. $$

::: {.column-margin}
The second identity follows from the first when $\calX$ is
...
*function* --- where $F$ need not be onto --- the second can fail.
:::
...
**IMPORTANT:**
Just as a one-way function is only hard to invert on a *randomly sampled input*,
...
### Digital signatures from trapdoor one-way permutations

This construction is called "full-domain hash." [@BR93]
...
- a hash function $H \colon \zo^* \to \calX$,
...
- $\Gen$ -- Just run the key-generation algorithm for the
...
Notice that the use of a hash function here is **critical** to
...
See [Boneh-Shoup Chapter 13.3](https://toc.cryptobook.us/book.pdf#page=550)
...
- $\Gen(1^\lambda) \to (\sk, \pk)$.
  ...
  or $\lambda = 2048$.
  :::
  - Sample two random $\lambda$-bit primes $p$ and $q$
...
- $F(\pk = N, x) \to y$.
  - The input space for the RSA function is
...
- **Given:** A polynomial $p(X) \deq X^5 - y \in \Z_N[X]$,
  for $y \getsr \Z^*_N$.
- **Find:** A value $x \in \Z^*_N$ such that $p(x) = 0 \in \Z^*_N$.

So the attacker must find the root of a polynomial modulo a composite integer $N$.

::: {.column-margin}
We work in $\Z_N^*$ rather than $\Z_N$ because
...
- **Factor $N$ into primes** and find a root modulo each of the primes.
  ...
  ::: {.column-margin}
  For a $2048$-bit modulus $N$ (that is,
...
- **Find a root over the integers** and reduce it modulo $N$.
  ...
  **Is inverting the RSA function as hard as factoring the modulus?**
  ...
  $p(X) \deq X^2 - y \bmod N$ for random $y \getsr \Z_N$ *is* as
...
in $\Z_N = \{0, 1, 2, \dots, N-1\}$ that are relatively prime to $N$.
When $p$ is prime $\phi(p) = p-1$.
The function $\phi(\cdot)$ is called *Euler's totient function*.

::: {.column-margin}
Two
natural numbers are *relatively prime* if they share no prime factors.
:::
...
$0$ and the multiples of $p$ and $q$:
$$ p, 2p, 3p, \dots, (q-1)p,\ \ \ q, 2q, 3q, \dots, (p-1)q.$$
So there are $N - 1 - (q-1) - (p-1) = (p-1)(q-1)$ numbers
...
- $I(\sk, y) \to x$.
  - The secret key $\sk$ consists of the prime factors $p,q$ of $N$.
...
\end{aligned}$$
for some $k \in \Z$, by @lem-inv, and
$$\begin{aligned}
      &= x \cdot (x^{\phi(N)})^k &&\bmod N\\
      &= x &&\bmod N,
\end{aligned}$$
by @thm-euler.
We could write $5d = k \phi(N) + 1$ because from @lem-inv,
we know that $5d \equiv 1 \bmod \phi(N)$.

**Using other public exponents.**
...
That algorithm is more general, but the one we used in @lem-inv
is simpler and is self-contained.

**Inverting RSA is easy on a negligible fraction of points.**
Recall the RSA is
If the preimage under the RSA function of a point $y$ is very very small,
then
If $x < N^{1/5}$, then computing $x$ given $y = x^5 \bmod N$ is *easy*.

**Is inverting RSA as hard as factoring the modulus $N$?**
The inversion algorithm we showed here requires knowing the prime factors
of the modulus $N$.
Inverting RSA is thus *no harder than* factoring $N$.

Is inverting RSA *as hard as* factoring $N$?
...
But for all we know, it could be that computing fifth roots modulo $N$ is *easier*
