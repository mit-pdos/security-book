...
First, we explored *privilege separation* as a way to architect
a system so that bugs do not lead to catastrophic security failures.
Next, we looked at *bug finding*---techniques to find and eliminate
bugs in source code before we use the code in production.
Now, we will discuss *runtime defenses*: how to detect buggy behavior
...
- figure out how to avoid *false positives* (erroneously halting a program
  when there is no bug), and
- try to implement these runtime defenses with minimal overhead,
...
The `gets` function in C will read from `stdin` until
it finds a NULL (`\0`) character in the input string.
If the string has length $>127$, the `gets` function could
copy adversarially generated input byte onto the stack, overwriting
the return address of the function `f`.
...
### Non-Executable Stack {#sec-overflow-nx}
...
some adversarial code on the stack (e.g., in the buffer `buf` in the
...
To try to remove the adversary's ability to overwrite the return address in the presence of a buffer overflow, another defense is to insert a *stack canary* in every stack frame between the function variables and the return address.
...
- An attacker can corrupt *data* on the stack, even if it does not
  corrupt the return address. This could be very bad for the program's behavior.
- An attacker might find a way to read the canary from memory (e.g.,
...
- In a forking web server, the child process may have the same canary value
  as the parent process. An attacker can potentially exploit this
  to learn the canary (See [@bittau2014hacking]).
...
return-oriented programming attack (@sec-overflow-nx).

::: {.column-margin}
Implementing ASLR requires compiler support.
...
### Bounds Checking with Fat Pointers
...
One way to implement bounds checking in C is a technique called *fat pointers*.
When using fat pointers, the compiler changes the representation of a
pointer to include not only an address, but also the *base* and the
*limit* of the buffer the pointer points to.
...
using `void* ptr = malloc(128)`, the compiler will
associate values `base = ptr` and `limit = ptr+128`
with the pointer `ptr`.
If we then assign `ptr3 = ptr + 3`, then the new
pointer `ptr3` will have the same base and limit
as `ptr`:
...
When the program deferences `ptr3`, the compiler will insert
checks to ensure that $\mathtt{base} \leq \mathtt{ptr3} < \mathtt{limit}$
and crash the program otherwise.

One limitation of fat pointers is that they only check
overflowing an *allocated region of memory*---not overflows
*within* a region of memory.
For example, if we use `malloc` to allocate a C `struct`,
overflowing the `buf` member of the struct could allow the
...
If there are function pointers in the `struct`, the result
of this within-region overflow could be as bad as overflowing the return
address.

Unlike a nonexecutable stack, stack canaries, and ASLR, fat pointers are not widely used. This is largely because the modified "fat" pointers
...
If the programmer cast a pointer to an `int` and back again,
...
takes exactly 64 bits to represent (e.g., as a field within a `struct`);
a compiler using fat pointers will break such programs.

### Control-Flow Integrity (CFI)

The goal of techniques for *control-flow integrity* is to limit the set
...
### Unsolved program: Use-after-free bugs

The defenses in this section have made buffer overflows extremely challenging to
exploit in modern code on modern systems.
These defenses do very little to prevent against *use-after-free* bugs,
in which a program `free`s allocated memory and then inadvertently
reads or writes the `free`d pointer.

## Taint Tracking to Defend against Input-Sanitization Bugs
...
A hypothetical Python example of how taint-tracking might prevent
SQL injection
:::

Many browsers implement taint tracking to prevent cross-site scripting using *Trusted Types*: for JavaScript calls that update the displayed HTML, such as `innerHTML = foo`, browsers may restrict the type of `foo` to ensure that the code explicity converts its type to something like `TrustedHTML`. This does not guarantee that the sanitization was done correctly, but does ensure that the programmer acknowledged the risk in their code.

### Taint tracking in language runtimes

Some programming languages associate a *taint bit* with every string
in the program.
...
### Taint tracking in operating systems
