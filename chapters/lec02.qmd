...
*who* issued the request. Then, we can
...
1. **Authenticate:** Identify the person
   or machine (the "*principal*") making the request.
2. **Authorize:** Decide if the principal
   is authorized to make the request.
3. **Audit:** Log some information about what
...
typing a PIN into a phone or a password into a computer terminal
or bank ATM machine. We will
...
![Three common security goals for client-server authentication systems: (a) direct attack, (b) eavesdropping attack, and (c) active attack.](/figs/auth-attacks.svg){#fig-auth-attacks width=90%}

@fig-auth-attacks sketches three types of attacks
on authentication systems. In more detail these
are, in increasing order of strength are:

- *Direct attack.*
  The attacker tries to impersonate the honest user, even though
  the attacker has never seen the target user authenticate.
  PIN-based and password-based authentication systems
  typically aim to defend only against direct attacks.
  For example, if your phone has a screen-lock PIN, it defends against
  an attacker that finds your phone on the street
  and tries to unlock it by guessing your PIN.

  At the same time, PIN-based authentication
  systems do _not_ protect against an attacker that can look over your
  shoulder while you are typing your password. These systems only aim to provide
  security when the attacker *never* sees you (the honest user) authenticate.

- *Eavesdropping attack.*
  A stronger security goal for authentication
  systems is to defend against _eavesdropping
  attacks_.
  Here, the attacker observes an honest user authenticating many times
  and then tries to impersonate that user.
  For example, an attacker looks over the shoulder
  of an honest user authenticating many times, and
  then tries to impersonate the honest user.
  Or, an attacker sees all of the traffic between the client and server,
  and then tries to impersonate the client.
  ...
  will not be able to use it to authenticate as you; it is a *one-time* password.

- *Active attack.*
  The strongest security goal that we will
  consider is security against an _active attack_.
  Here, we envision an attacker that compromises the server, 
  then interacts many times with the honest user,
  and finally—after the server is restored to a good state—tries to authenticate
  as the honest user (*active attack*).

  Servers on the web are often designed to protect
  against active attacks: even if an attacker can
  break into one of Google's front-end web
  servers interacts with authenticating clients, the 
  attacker should not be able to authenticate as
  those clients to other Google services 

  U2F security keys, and other schemes based on digital
  signatures (@sec-sig), aim to protect against active attacks.
...
three increasingly strong types of authentication,
that provide these three increasingly strong
security properties.

## Protecting against direct attacks: Bearer tokens, PINs, and passwords

![A bearer-token-based authentication scheme. To authenticate, the client sends its secret key $k$ to the server. [WARNING: In practice, the server should store the secret $k$ under a slow hash function (see @sec-auth-pwhash).]{style="color: red;"}](/figs/auth-direct.svg){#fig-auth-direct width=70%}

The simplest form of authentication uses secret
passwords or PINs. We sometimes call these secret values
*bearer tokens*; whoever bears (holds)
...
3. The server checks the password against
...
Fancier authentication systems require the client to run 
complex cryptographic algorithms, which are too
difficult for most humans to compute in their brains.

Bearer-token-based schemes _do_ provide security against direct attacks:
if a direct attacker has never seen the client authenticate, the
attacker's best strategy is to just guess the client's password.
Thus, the security of these schemes against direct attacks depends
entirely on the adversary's uncertainty about the client's password.
...
![The most common passwords, according to NordPass (<https://nordpass.com/most-common-passwords-list/>), sorted by their frequency descending. A small number of common passwords dominate.](/figs/password-dist.svg){width=100%}

The security of a password-based authentication system
rests entirely on the attacker's inability to guess the client's 
password in a small number of guesses.

*Entropy* is a way to quantify an adversary's
...
::: {.column-margin}
There are many definitions of entropy, which are
all slightly different from each other. We are
being very informal here, so we will call them
all _entropy_ and not worry about the fine
distinctions between the different notions.
:::
...
If a system generated a truly random $b$-bit password for each user,
each password would be indeed equally likely and
it would take the attacker roughly $2^b$ time to
guess the user's password.
But then it becomes very difficult for a user to remember their
password.

**Common passwords are easy to guess.**
...
A typical human might sample their password
from a distribution with roughly 20 bits of
...
### Dealing with weak passwords

*Every* system that uses password- or PIN-based authentication
must contend with the fact that most passwords are not that difficult
for an attacker to guess.

In this section, we describe some common ways to 
mitigate the fact that users pick weak passwords. 
Each of these mitigations is flawed,
but each is better than nothing. Each makes the attacker's
...
**Aggressively limit the number of guesses.**
When using passwords as an authentication
mechanism, an authentication system must *always*
...
Limiting the number of guesses effectively prevents a *single*
...
can often compromise *some* account on the machine if it is allowed
10 guesses at *every* user's password.
...
**Try to coerce users into picking stronger passwords.**
...
they might say that `6175551212` is a great password; if the attacker
...
- *Require longer passwords.* If someone tries to use
  `abc123` as a password but it's not long
  enough, they might use `abc123456`---but
...
- *Prohibit using common English words in passwords.*
...
### Avoiding weak passwords with a password manager
...
the password manager can sample these
passwords truly at random from a high-entropy distribution.
...
schemes provide *no security* against eavesdropping or active attacks.
...
### Password hashing: Trying to get some protection against server compromise {#sec-auth-pwhash}
...
Since common passwords are easy to guess, avoiding
...
*salted cryptographic password-hashing function*.
...
the server chooses a random 128-bit string, called a *salt*,
...
A rainbow table is essentially a compressed table $(\mathtt{passwd}, H(\mathtt{passwd}))$
pairs, where $H$ is a common hash function, and $\mathtt{passwd}$ ranges over a
...
any reasonably large fraction of the $(\mathtt{salt}\|\mathtt{passwd})$ pairs.
:::
...
- The password-hashing function $H$ is designed to be relatively expensive
  ...
  such as SHA256).
- The use of a per-user random salt ensures that guesses at one user's password
  are useless in inverting another user's password hash.
  Salting also defeats *precomputation attacks*, in which an attacker
...
## Protecting against eavesdropping attacks: Challenge-response protocols
...
![A challenge-response-based authentication system. The server and client share a secret key $k$. To authenticate, the client sends a function $F$ of its secret key and the server's challenge $c$.](/figs/auth-eavesdropping.svg){#fig-auth-eaves width=80%}

@fig-auth-eaves describes an über-simplified challenge-response
...
1. The server chooses a long random string $c$, which we call a *challenge*
   and sends it to the authenticating client.
2. The client computes an authentication "tag" $t \gets F(k, c)$, where $F(\cdot, c)$ is hard to compute without knowing $k$.
   (The function $F$ here is a Message Authentication Code,
   which we will talk more specifically about in @sec-mac.)
   The sends the MAC tag $t$ to the server.
3. The server receives a tag $t'$ from the client and ensures that $t' = F(k,c)$.
...
protocol. The only difference from @fig-auth-eaves is that in a TOTP
...
generate this code---something like $F(k, \mathtt{gettimeofday()\ /\ 30})$.
The phone can generate the code, display it to the user, and the server
can then verify the code by recomputing it.

### Authenticating requests

Often, a client will want to send an *authenticated message* to a server.
That is, the client often wants to simultaneously authenticate to the server
and send a request $\mathsf{req}$, such as $\mathsf{req} = \mathtt{rm\ file.txt}$.
...
An **unsafe** way for the client to simultaneously authenticate to the
...
### Phishing attacks (attacker-in-the-middle attacks)

A *phishing* attack is one
in which an attacker tricks a user into giving away their Gmail password,
for example, by creating a website that looks, for example, like the `gmail.com`
login page.
...
## Protecting against active attacks: Signatures and U2F

![An authentication scheme based on digital signatures.](/figs/auth-active.svg){#fig-auth-active width=70%}

To provide security against active attacks, we can use an authentication
scheme based on digital signatures, which we will discuss in @sec-sig.
...
In particular, the server *does not store any secrets*---even if the attacker
...
In particular, the U2F software on the client passes the name of the server (e.g., `amazon.com`),
in addition to a server-provided random challenge $c$, to the U2F token.
The token then produces a signature on the string $c\|\mathtt{amazon.com}$.
If the attacker sets up `amason.com` and gets the user to visit it,
the U2F device will only generate a code that is good for `amason.com`
and not the real `amazon.com`.
