...
and "encrypted pipe" between a client and a server.
...
## Problem: Encryption does not hide the source and destination of a packet
...
(In some ways, the "pipe" analogy fits
here: anyone can see where the pipe starts and
where it ends.)

**Solution Attempt: Tor.**
...
::: {.column-margin}
It is difficult to evaluate the security of a tool
...
the Tor client will first select three *relays* from the Tor network $(A, B, C)$.
...
3. Through that tunnel-inside-a-tunnel, open an encrypted tunnel to the
...
*circuit* from relay $A$ to $B$ to $C$ to the
...
## Problem: Attacker sees packet sizes and timings
...
- *Watching a movie:* Video traffic has a distinct traffic pattern.
  By monitoring, for example, the length of time that a client spends
  watching a movie, a network attacker learn with
  fairly high accuracy *which movie* the client is watching.
- *Using `ssh`:*
...
1. **Random Noise:**
   ...
   Unfortunately, a patient attacker can use *averaging*
...
## A Promising Direction: Metadata Privacy for Messaging
...
an adversary can compromise the communication *endpoints*.
...
- *Correctness:* The client outputs $x_i \in \bin$.
- *Security:* The server "learns nothing" about the
...
**Naïve private information retrieval.**
...
### A non-trivial private-information-retrieval scheme.
