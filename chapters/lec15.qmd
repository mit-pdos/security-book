...
useful to *isolate* different components of
a computer system.
...
::: {.column-margin}
Computer systems for high-stakes
...
*state* of each domain, $S_A$ and $S_V$. For
...
We would also like *confidentiality*: an adversarial domain should
not be able to read a data from any other domain.
We can formalize this by considering two worlds,
each with a *different* victim state.
...
### Non-interference: Strong confidentiality and integrity
...
This definition is often called *non-interference*. We can similarly strengthen our definition of integrity by requiring that $S_V'$ is identical after running $V$ whether or not $A$ is run.
$$ (S_A, S_V) \xrightarrow[A, V, V, A, A]{\text{run}\, A, V} (-, S'_V) $$
$$ (S_A, S_V) \xrightarrow[A, A, \ldots, A]{\text{run only}\, V} (-, S'_V) $$

### Non-interference is difficult to achieve
...
and victim *share limited resources*---such as CPU, RAM,
...
::: {.column-margin}
The information leakage across isolation boundaries
as a results of resource contention is one type
of *side channel* or *covert channel*.
...
- **Strictly partition resources to prevent contention.**
...
- **Prevent isolation domains from detecting resource contention.**
  ...
  no networking, etc.---then programs may not be able to *detect*
...
3. ensure that state-modifying operations
...
### Example: Isolation in Virtual Machines
...
1. **What is the isolation domain's state?**
...
3. **How does the virtual-machine monitor ensure isolation?**
   - To handle updates to the register state, the monitor
     uses *time multiplexing*: one virtual machine
     runs on the CPU at a time and controls the
     CPU's registers.
   - To handle accesses to memory, the monitor uses
     *naming/translation* via virtual memory.
   - To handle device operations, the monitor uses
     *emulation*.
...
`b[c]` does not access out-of-bounds memory:
...
function calls.
When calling the function `f` in the code snippet above,
...
If an isolated program can set the value in `*f`
to the location `(***)` in the snippet above,
