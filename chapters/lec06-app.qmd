# Factoring integers {#sec-fact}

{{< include /macros/_macros.qmd >}}

The problem of integer factorization was central to 20th-century cryptography.
Breaking the one-wayness of the RSA trapdoor one-way function (@sec-rsa), for example,
...
Trial division is an *exponential time* algorithm, since it runs in time $2^{\Omega(n)}$,
where the bitlength $n$ is the size of the number to be factored.
The best known factoring algorithms run in *sub-exponential time* $2^{O(n^c)}$,
...
::: {.column-margin}
In this
discussion, we draw on Arjen Lenstra's very nice survey on factoring [@lenstra2000integer].
:::
...
Once we have these squares, we can use them to factor as in @sec-fact-bg
...
**Input:** An integer $N = pq$ for odd primes $p$ and $q$. A parameter $B \in \N$,
which we refer to as "the size of the factor base."

**Output:** The factors $(p,q)$ of $N$.

1. **Collect linear relations.**
   ...
   - If successful, add the pair $(r, (e_2, e_3, e_5, \dots))$
     to the list $L$.
...
2. **Solve linear system.**
   ...
   ::: {.column-margin}
   We can find the set $S$ using Gaussian elimination
   in roughly $B^3$ time.
...
3. **Use Euclid's algorithm to try to factor $N$.**
   We can take:
   $$\begin{aligned}
...
2. Solving the linear system using Gaussian elimination
   takes roughly $B^3$ time.

3. Run Euclid's algorithm---the time required here is
...
very nice survey on smooth numbers. [@granville2008smooth]
:::
...
Now we can plug this estimate into the expression (@eq-time)
