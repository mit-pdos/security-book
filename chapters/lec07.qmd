...
the mapping of **human-intelligible names** to
...
$$ \mathsf{IsKeyFor}(\vk, \mathtt{name}) \to \zo.$$
That is, given a verification key $\vk$ and a name $\mathtt{name}$,
the public-key infrastructure gives a way to check whether this mapping
is valid.

We now discuss some ways to implement this API.

## Option 1: Use verification keys as names
...
The public key *is* name of that account.
...
The secure shell system (`SSH`) uses TOFU for key management by default.
...
That is, the client will accept the *first* verification key
...
- If the first key that client receives for a particular name
...
These entities are called *Certificate Authorities* (CAs).
...
Whenever the owner of website `example.com`, for example, generates
a new public key $\vk_\mathtt{example.com}$, the website owner can ask a
certificate authority to certify that $\vk_\mathtt{example.com}$ really belongs to `example.com`.
The certificate authority does this by signing the
pair $(\mathtt{example.com}, \vk_\mathtt{example.com})$
using its own signing keypair $\vk_\text{CA}$
to generate a signature $\sigma_\text{CA}$.
This signed attestation $(\mathtt{example.com}, \vk_\mathtt{example.com}, \sigma_\text{CA})$
is called a *certificate*.

::: {.column-margin}
In practice the structure of certificates is much more complicated
than we are showing here, and include all sorts of additional metadata. But the
basic idea is the same as we describe here.
:::

When a client connects to `example.com`, the server at `example.com`
will supply the client with the certificate $(\mathtt{example.com}, \vk_\mathtt{example.com},\sigma_\text{CA})$.
...
the verification key $\vk_\mathtt{example.com}$ really belongs to `example.com`.
...
- For each $\vk_\text{CA}$ in $\mathsf{caKeys}$:
  - If $\Ver(\vk_\text{CA}, (\mathtt{name}, \vk), \sigma) = 1$: Return true.
- Return false.
:::
...
::: {.column-margin}
In order to use TLS on a website you own, you need to convince one of the certificate authorities to give you a certificate---i.e., to sign your $(\mathtt{name}, \vk)$ pair.
To do so, the CA will have some protocol to follow---typically, you will send your $(\mathtt{name}, \vk)$ pair to the CA, who will then ask you to verify that you own the name somehow. This is called *domain validation*.
...
Once the CA is convinced that you own the domain, the CA will reply with a certificate: a signature over the tuple $(\mathtt{name}, \vk)$. This $(\mathtt{name}, \vk, \sig_{\text{CA}})$.
:::
...
- A client can choose which CAs to trust
...
- If a server's private key gets stolen,
  there is no great plan for *revoking* or updating a name-to-key mapping.
...
A standard protocol for validating domain ownership is ACME^[<https://tools.ietf.org/html/rfc8555#section-8.3>].

### Revocation

In many cases, a CA will want to delete or change a name-to-key mapping.
This process is called *certificate revocation*.
There are several possible reasons for this:

- The owner of a verification key may have their corresponding secret key
  be lost or stolen.
- A company may want to rotate keys,
...
- Software bugs may lead a user to generate an
  insecure keypair that they later want to revoke. [@yilek2009private; @nemec2017return]
